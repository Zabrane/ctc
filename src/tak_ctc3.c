/* tak_ctc3.c
 * tak_dtc converted to call-threading
 */
...
#define GET1(TY)	*(TY*)(GET_ESP() + 4)
#define GET2(TY)	*(TY*)(GET_ESP() + 8)
...
static void run(code_ptr_t pc0)
{
	...
	END_ENTRY;
	continue;
	BEGIN_ENTRY2(op_call,code_ptr_t,callee,code_ptr_t,ra);
	{
	    PUSH_RA(ra);
	    NEXT1(callee);
	}
	END_ENTRY;
	continue;
	BEGIN_ENTRY1(op_jmp,code_ptr_t,npc);
	{
	    NEXT1(npc);
	}
	END_ENTRY;
	continue;
	BEGIN_ENTRY1(op_jgt,code_ptr_t,npc);
	{
	    int x, y;
	    POP_INT(x);
	    POP_INT(y);
...
extern __opcode code_ptr_t op_call(code_ptr_t, code_ptr_t);
extern __opcode code_ptr_t op_jmp(code_ptr_t);
extern __opcode code_ptr_t op_jgt(code_ptr_t);
...
#include "ctc_c_shared_naive.h"
#include "ctc_x86_shared_naive.h"
#include "ctc_amd64_shared_naive.h"
#include "ctc_ppc_shared_naive.h"
#include "ctc_sparc_shared_naive.h"
#include "ctc_arm_shared_naive.h"
