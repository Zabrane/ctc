 ...
 * - passing instruction operands in registers
 */
...
#define OP_GET(n)	"movl\t$" STRING(n) ", %%eax\n\tcall\top_get\n\t"
#define OP_SET(n)	"movl\t$" STRING(n) ", %%eax\n\tcall\top_set\n\t"
#define OP_SUB1()	"call\top_sub1\n\t"
#define OP_RET(n)	"movl\t$" STRING(n) ", %%eax\n\tcall\top_ret\n\tjmp\t*%%eax\n\t"
#define OP_CALL(l1,l2)	"movl\t$" LABEL(l2) ", %%eax\n\tcall\top_push_ra\n\tjmp\t" LABEL(l1) "\n\t"
#define OP_JMP(lab)	"jmp\t" LABEL(lab) "\n\t"
#define OP_JGT(lab)	"call\top_jgt\n\ttestl\t%%eax, %%eax\n\tjg\t" LABEL(lab) "\n\t"
#define OP_PUSH(n)	"movl\t$" STRING(n) ", %%eax\n\tcall\top_push\n\t"
